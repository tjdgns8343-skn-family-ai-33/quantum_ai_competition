"""Vectorized exact statevector and adjoint gradients for the C1O circuit."""
...
_Z_READOUT_SIGN = np.where(
    ((_BASIS >> C1O_READOUT_QUBIT) & 1) == 0, 1.0, -1.0
)
...
def _validate_inputs(x: np.ndarray, weights: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    if features.ndim != 2 or features.shape[1] != C1O_N_FEATURES:
        raise ValueError(f"C1O expects shape (n, {C1O_N_FEATURES}); got {features.shape}.")
    if parameters.shape != (C1O_N_WEIGHTS,):
        raise ValueError(f"C1O expects {C1O_N_WEIGHTS} weights; got {parameters.shape}.")
    if not np.isfinite(features).all() or not np.isfinite(parameters).all():
        raise ValueError("C1O inputs and weights must be finite.")
...
def _forward_with_operations(
    ...
    before = state
    readout_parameter_index = C1O_N_WEIGHTS - 1
    angle = float(parameters[readout_parameter_index])
    state = _apply_ry(state, angle, C1O_READOUT_QUBIT)
    operations.append(
        _Operation(
            kind="ry",
            before=before,
            angle=angle,
            qubit=C1O_READOUT_QUBIT,
...
def c1o_exact_probabilities(x: np.ndarray, weights: np.ndarray) -> np.ndarray:
...
def _balanced_sample_weights(y: np.ndarray) -> np.ndarray:
    ...
    if not np.isin(labels, (0, 1)).all():
        raise ValueError("C1O labels must be binary 0/1 values.")
    return result


def c1o_balanced_bce_value_and_gradient(
    ...
    gradient = np.zeros(C1O_N_WEIGHTS, dtype=float)
    ...
    for operation in reversed(operations):
        ...
        if operation.kind == "ry":
            ...
            raise RuntimeError(f"Unknown C1O operation: {operation.kind}")
