"""End-to-end training for the compliant E1S quantum circuit."""
...
from .e1s_circuit import (
    ...
    E1S_REUPLOAD_BLOCKS,
    build_e1s_unitary,
    export_e1s_submission_qasm,
)
from .e1s_simulator import (
...
@dataclass(frozen=True)
class E1SCrossValidationConfig:
    train_csv: Path
    artifacts_dir: Path = Path("artifacts")
    split_seed: int = 2026
    init_seed: int = 2026
...
def e1s_random_initial_point(
    ...
    if local_scale < 0 or affine_scale_jitter < 0:
        raise ValueError("E1S initialization scales must be non-negative.")
    rng = np.random.default_rng(seed)
    weights = rng.uniform(-local_scale, local_scale, E1S_N_WEIGHTS)
    for feature_index in range(E1S_N_FEATURES):
        offset = 4 * feature_index
        weights[offset] = rng.uniform(
            1.0 - affine_scale_jitter, 1.0 + affine_scale_jitter
        )
        weights[offset + 2] = rng.uniform(
            1.0 - affine_scale_jitter, 1.0 + affine_scale_jitter
        )
    return weights
...
def _stratified_subsample(
    ...
    if max_rows < 20:
        raise ValueError("E1S max_rows must be at least 20.")
    rng = np.random.default_rng(seed)
...
def _e1s_bounds() -> list[tuple[float, float]]:
    bounds = [(-np.pi, np.pi) for _ in range(E1S_N_WEIGHTS)]
    for feature_index in range(E1S_N_FEATURES):
        offset = 4 * feature_index
        bounds[offset] = (-3.0, 3.0)
        bounds[offset + 2] = (-3.0, 3.0)
    return bounds


def optimize_e1s_quantum_loss(
    x: np.ndarray,
    y: np.ndarray,
    *,
    seed: int,
    ...
    """Optimize analytic gradients of the E1S statevector probability only."""
    initial = e1s_random_initial_point(
        seed,
    ...
    def value_and_gradient(weight_values: np.ndarray) -> tuple[float, np.ndarray]:
        ...
        value, gradient = e1s_balanced_bce_value_and_gradient(
            x, y, weight_values
        )
    ...
    initial_loss, initial_gradient = e1s_balanced_bce_value_and_gradient(
        x, y, initial
    )
    result = minimize(
        value_and_gradient,
    ...
    return selected, {
        ...
        "objective": "balanced_binary_cross_entropy_of_E1S_q0_probability",
...
def _metrics(
    ...
    *,
    ...
    exact = e1s_exact_probabilities(x, weights)
    rng = np.random.default_rng(seed)
...
def e1s_feature_causal_report(
    ...
    for feature_index in range(E1S_N_FEATURES):
        ...
        sensitivity = np.abs(
            (e1s_exact_probabilities(plus, weights) - e1s_exact_probabilities(minus, weights))
            / (2.0 * perturbation)
        )
        rows.append(
            {
...
def _config_payload(config: E1STrainConfig) -> dict:
    return {
        **asdict(config),
...
def run_e1s_screen(config: E1STrainConfig) -> Path:
    ...
    weights, optimization, initial = optimize_e1s_quantum_loss(
        screen_x[fit],
        screen_y[fit],
        seed=config.seed,
    ...
    run_dir = config.artifacts_dir / (
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_screen_e1s_"
        f"split{split_seed}_init{config.seed}"
    )
    run_dir.mkdir(parents=True, exist_ok=False)
    payload = {
        "mode": "train_only_holdout_E1S_direct_quantum_gradient_screen",
        "architecture": E1S_ARCHITECTURE,
        ...
        "qiskit_equivalence": e1s_qiskit_equivalence_report(screen_x[valid], weights),
        "initial_feature_causal_report": e1s_feature_causal_report(screen_x, initial),
        "trained_feature_causal_report": e1s_feature_causal_report(screen_x, weights),
...
def _stratified_folds(
    ...
    if folds < 2:
        raise ValueError("E1S cross-validation requires at least two folds.")
    rng = np.random.default_rng(seed)
...
def run_e1s_cross_validation(config: E1SCrossValidationConfig) -> Path:
    """Generate genuine E1S OOF probabilities and a train-only threshold."""
    x, y, data_info = load_raw_train(config.train_csv)
    ...
    for fold_index, (fit, valid) in enumerate(splits):
        weights, optimization, _ = optimize_e1s_quantum_loss(
            x[fit],
            y[fit],
        ...
        probability = e1s_exact_probabilities(x[valid], weights)
        ...
        equivalence_reports.append(e1s_qiskit_equivalence_report(x[valid], weights))

    if not np.isfinite(oof_probability).all():
        raise RuntimeError("E1S OOF generation left one or more rows unpredicted.")
    ...
    run_dir = config.artifacts_dir / (
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_cv_e1s_"
        f"split{config.split_seed}_init{config.init_seed}"
    )
    run_dir.mkdir(parents=True, exist_ok=False)
    payload = {
        "mode": "E1S_quantum_only_out_of_fold_threshold_selection",
        "architecture": E1S_ARCHITECTURE,
        "submission_created": False,
        "config": {
            **asdict(config),
        ...
        "threshold_source": "out_of_fold_E1S_quantum_probabilities_only",
...
def run_e1s_final(config: E1STrainConfig) -> Path:
    x, y, data_info = load_raw_train(config.train_csv)
    if config.max_rows is not None and config.max_rows < len(y):
        raise ValueError("E1S final training must use the complete public train set.")
    if not 0.0 < config.decision_threshold < 1.0:
        raise ValueError("E1S decision_threshold must be in (0, 1).")
    weights, optimization, initial = optimize_e1s_quantum_loss(
        x,
        y,
        seed=config.seed,
    ...
    run_dir = config.artifacts_dir / (
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_final_e1s_seed{config.seed}"
    )
    run_dir.mkdir(parents=True, exist_ok=False)
    constraint = export_e1s_submission_qasm(run_dir / "classifier.qasm")
    weights_payload = {
    ...
    expected_feature_uses = {
        f"x_{feature_index}": 2 * sum(
            feature_index in block for block in E1S_REUPLOAD_BLOCKS
        )
        for feature_index in range(E1S_N_FEATURES)
    }
    provenance = {
        "architecture": E1S_ARCHITECTURE,
        "weight_count": E1S_N_WEIGHTS,
        "parameter_generation": "label_independent_random_initialization_then_direct_E1S_quantum_probability_gradient_optimization",
        "initialization": {
            "seed": config.seed,
        ...
        "loss_source": "E1S q0 measurement probability versus raw public_train label",
        "loss": "balanced binary cross-entropy",
        "optimizer": "L-BFGS-B with exact adjoint quantum-circuit gradient",
        "selected_raw_features_zero_based": list(range(E1S_N_FEATURES)),
        "selected_raw_features_csv": [f"x{index + 1}" for index in range(E1S_N_FEATURES)],
    ...
    metrics = {
        "mode": "full_train_E1S_direct_quantum_gradient_optimization",
        "architecture": E1S_ARCHITECTURE,
        "config": _config_payload(config),
        "data": data_info,
        "optimization": optimization,
        ...
        "qiskit_equivalence": e1s_qiskit_equivalence_report(x, weights),
        "initial_feature_causal_report": e1s_feature_causal_report(x, initial),
        "trained_feature_causal_report": e1s_feature_causal_report(x, weights),
    }
    (run_dir / "training_metrics.json").write_text(
        json.dumps(metrics, indent=2), encoding="utf-8"
    )
    note = (
        "E1S shared dual-axis affine VQC uses all raw CSV features with no preprocessing or augmentation. "
        "Blocks 1/3 encode x1-x4 and blocks 2/4 encode x5-x8 using shared single-feature affine "
        "RY(theta_y_scale*x_i + theta_y_bias) and RZ(theta_z_scale*x_i + theta_z_bias) gates. "
        "Each block then applies a trainable RY mixer "
        ...
        "balanced cross-entropy of the exact E1S q0 statevector probability on public_train.csv. "
