"""End-to-end training for the compliant D1S quantum circuit."""
...
from .d1s_circuit import (
    D1S_ARCHITECTURE,
    D1S_N_FEATURES,
    D1S_N_WEIGHTS,
    D1S_REUPLOAD_BLOCKS,
    build_d1s_unitary,
    export_d1s_submission_qasm,
)
from .d1s_simulator import (
    d1s_balanced_bce_value_and_gradient,
    d1s_exact_probabilities,
)
...
@dataclass(frozen=True)
class D1SCrossValidationConfig:
    train_csv: Path
    artifacts_dir: Path = Path("artifacts")
    split_seed: int = 2026
    init_seed: int = 2026
...
def d1s_random_initial_point(
    ...
    if local_scale < 0 or affine_scale_jitter < 0:
        raise ValueError("D1S initialization scales must be non-negative.")
    rng = np.random.default_rng(seed)
    weights = rng.uniform(-local_scale, local_scale, D1S_N_WEIGHTS)
    for block_index in range(len(D1S_REUPLOAD_BLOCKS)):
        offset = 6 * block_index
        weights[offset : offset + 2] = rng.uniform(
...
def _stratified_subsample(
    ...
    if max_rows < 20:
        raise ValueError("D1S max_rows must be at least 20.")
    rng = np.random.default_rng(seed)
...
def _d1s_bounds() -> list[tuple[float, float]]:
    bounds = [(-np.pi, np.pi) for _ in range(D1S_N_WEIGHTS)]
    for block_index in range(len(D1S_REUPLOAD_BLOCKS)):
        offset = 6 * block_index
...
def optimize_d1s_quantum_loss(
    ...
    *,
    ...
    """Optimize analytic gradients of the D1S statevector probability only."""
    initial = d1s_random_initial_point(
        seed,
    ...
    def value_and_gradient(weight_values: np.ndarray) -> tuple[float, np.ndarray]:
        ...
        value, gradient = d1s_balanced_bce_value_and_gradient(
            x, y, weight_values
        )
    ...
    initial_loss, initial_gradient = d1s_balanced_bce_value_and_gradient(
        x, y, initial
    )
    result = minimize(
        value_and_gradient,
    ...
    return selected, {
        ...
        "training_emulator": "vectorized_exact_2_qubit_statevector",
        "qiskit_equivalence_required": True,
        "objective": "balanced_binary_cross_entropy_of_D1S_q0_probability",
...
def _metrics(
    ...
    *,
    ...
    exact = d1s_exact_probabilities(x, weights)
    rng = np.random.default_rng(seed)
...
def d1s_qiskit_equivalence_report(
    x: np.ndarray, weights: np.ndarray, *, max_rows: int = 16
) -> dict:
    sample = np.asarray(x[:max_rows], dtype=float)
    custom = d1s_exact_probabilities(sample, weights)
    circuit, features, parameters = build_d1s_unitary()
...
def _config_payload(config: D1STrainConfig) -> dict:
    return {
        **asdict(config),
...
def run_d1s_screen(config: D1STrainConfig) -> Path:
    ...
    weights, optimization, initial = optimize_d1s_quantum_loss(
        screen_x[fit],
        screen_y[fit],
        seed=config.seed,
    ...
    run_dir = config.artifacts_dir / (
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_screen_d1s_"
        f"split{split_seed}_init{config.seed}"
    )
    run_dir.mkdir(parents=True, exist_ok=False)
    payload = {
        "mode": "train_only_holdout_D1S_direct_quantum_gradient_screen",
        "architecture": D1S_ARCHITECTURE,
        ...
        "qiskit_equivalence": d1s_qiskit_equivalence_report(screen_x[valid], weights),
        "initial_feature_causal_report": d1s_feature_causal_report(screen_x, initial),
        "trained_feature_causal_report": d1s_feature_causal_report(screen_x, weights),
...
def _stratified_folds(
    ...
    if folds < 2:
        raise ValueError("D1S cross-validation requires at least two folds.")
    rng = np.random.default_rng(seed)
...
def run_d1s_cross_validation(config: D1SCrossValidationConfig) -> Path:
    """Generate genuine D1S OOF probabilities and a train-only threshold."""
    x, y, data_info = load_raw_train(config.train_csv)
    ...
    for fold_index, (fit, valid) in enumerate(splits):
        weights, optimization, _ = optimize_d1s_quantum_loss(
            x[fit],
            y[fit],
        ...
        probability = d1s_exact_probabilities(x[valid], weights)
        ...
        equivalence_reports.append(d1s_qiskit_equivalence_report(x[valid], weights))

    if not np.isfinite(oof_probability).all():
        raise RuntimeError("D1S OOF generation left one or more rows unpredicted.")
    ...
    run_dir = config.artifacts_dir / (
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_cv_d1s_"
        f"split{config.split_seed}_init{config.init_seed}"
    )
    run_dir.mkdir(parents=True, exist_ok=False)
    payload = {
        "mode": "D1S_quantum_only_out_of_fold_threshold_selection",
        "architecture": D1S_ARCHITECTURE,
        "submission_created": False,
        "config": {
            **asdict(config),
        ...
        "threshold_source": "out_of_fold_D1S_quantum_probabilities_only",
...
def run_d1s_final(config: D1STrainConfig) -> Path:
    x, y, data_info = load_raw_train(config.train_csv)
    if config.max_rows is not None and config.max_rows < len(y):
        raise ValueError("D1S final training must use the complete public train set.")
    if not 0.0 < config.decision_threshold < 1.0:
        raise ValueError("D1S decision_threshold must be in (0, 1).")
    weights, optimization, initial = optimize_d1s_quantum_loss(
        x,
        y,
        seed=config.seed,
    ...
    run_dir = config.artifacts_dir / (
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_final_d1s_seed{config.seed}"
    )
    run_dir.mkdir(parents=True, exist_ok=False)
    constraint = export_d1s_submission_qasm(run_dir / "classifier.qasm")
    weights_payload = {
    ...
    expected_feature_uses = {
        f"x_{feature_index}": sum(
            feature_index in block for block in D1S_REUPLOAD_BLOCKS
        )
        for feature_index in range(D1S_N_FEATURES)
    }
    provenance = {
        "architecture": D1S_ARCHITECTURE,
        "weight_count": D1S_N_WEIGHTS,
        "parameter_generation": "label_independent_random_initialization_then_direct_D1S_quantum_probability_gradient_optimization",
        "initialization": {
            "seed": config.seed,
        ...
        "training_quantum_emulator": "vectorized exact 2-qubit statevector verified against qiskit.quantum_info.Statevector",
        "loss_source": "D1S q0 measurement probability versus raw public_train label",
        "loss": "balanced binary cross-entropy",
        "optimizer": "L-BFGS-B with exact adjoint quantum-circuit gradient",
        "selected_raw_features_zero_based": list(range(D1S_N_FEATURES)),
        "selected_raw_features_csv": [f"x{index + 1}" for index in range(D1S_N_FEATURES)],
    ...
    metrics = {
        "mode": "full_train_D1S_direct_quantum_gradient_optimization",
        "architecture": D1S_ARCHITECTURE,
        "config": _config_payload(config),
        "data": data_info,
        "optimization": optimization,
        ...
        "qiskit_equivalence": d1s_qiskit_equivalence_report(x, weights),
        "initial_feature_causal_report": d1s_feature_causal_report(x, initial),
        "trained_feature_causal_report": d1s_feature_causal_report(x, weights),
    }
    (run_dir / "training_metrics.json").write_text(
        json.dumps(metrics, indent=2), encoding="utf-8"
    )
    note = (
        "D1S causal data-reuploading VQC uses all raw CSV features with no preprocessing or augmentation. "
        "Eight pair blocks encode (x1,x2), (x3,x4), (x5,x6), and (x7,x8) exactly twice as single-feature affine "
        "RY(theta_scale*x_i + theta_bias) gates. Each block then applies trainable RZ/RY mixers "
        "with one alternating-direction CX and a trainable RY mixer; each full upload ends with a shared RZ/RY mixer, and only q0 is measured. "
        "All theta values use label-independent random initialization and are optimized only against "
        "balanced cross-entropy of the exact D1S q0 statevector probability on public_train.csv. "
