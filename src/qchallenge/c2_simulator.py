"""Vectorized exact statevector and adjoint gradients for the C2 circuit."""
...
_Z_READOUT_SIGN = np.where(
    ((_BASIS >> C2_READOUT_QUBIT) & 1) == 0, 1.0, -1.0
)
...
def _validate_inputs(x: np.ndarray, weights: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    if features.ndim != 2 or features.shape[1] != C2_N_FEATURES:
        raise ValueError(f"C2 expects shape (n, {C2_N_FEATURES}); got {features.shape}.")
    if parameters.shape != (C2_N_WEIGHTS,):
        raise ValueError(f"C2 expects {C2_N_WEIGHTS} weights; got {parameters.shape}.")
    if not np.isfinite(features).all() or not np.isfinite(parameters).all():
        raise ValueError("C2 inputs and weights must be finite.")
...
def _forward_with_operations(
    ...
    before = state
    readout_parameter_index = C2_N_WEIGHTS - 1
    angle = float(parameters[readout_parameter_index])
    state = _apply_ry(state, angle, C2_READOUT_QUBIT)
    operations.append(
        _Operation(
            kind="ry",
            before=before,
            angle=angle,
            qubit=C2_READOUT_QUBIT,
...
def c2_exact_probabilities(x: np.ndarray, weights: np.ndarray) -> np.ndarray:
...
def _balanced_sample_weights(y: np.ndarray) -> np.ndarray:
    ...
    if not np.isin(labels, (0, 1)).all():
        raise ValueError("C2 labels must be binary 0/1 values.")
    return result


def c2_balanced_bce_value_and_gradient(
    ...
    gradient = np.zeros(C2_N_WEIGHTS, dtype=float)
    ...
    for operation in reversed(operations):
        ...
        if operation.kind == "ry":
            ...
            raise RuntimeError(f"Unknown C2 operation: {operation.kind}")
