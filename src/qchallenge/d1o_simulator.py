"""Vectorized exact statevector and adjoint gradients for the D1O circuit."""
...
_Z_READOUT_SIGN = np.where(
    ((_BASIS >> D1O_READOUT_QUBIT) & 1) == 0, 1.0, -1.0
)
...
def _validate_inputs(x: np.ndarray, weights: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    if features.ndim != 2 or features.shape[1] != D1O_N_FEATURES:
        raise ValueError(f"D1O expects shape (n, {D1O_N_FEATURES}); got {features.shape}.")
    if parameters.shape != (D1O_N_WEIGHTS,):
        raise ValueError(f"D1O expects {D1O_N_WEIGHTS} weights; got {parameters.shape}.")
    if not np.isfinite(features).all() or not np.isfinite(parameters).all():
        raise ValueError("D1O inputs and weights must be finite.")
...
def d1o_exact_probabilities(x: np.ndarray, weights: np.ndarray) -> np.ndarray:
...
def _balanced_sample_weights(y: np.ndarray) -> np.ndarray:
    ...
    if not np.isin(labels, (0, 1)).all():
        raise ValueError("D1O labels must be binary 0/1 values.")
    return result


def d1o_balanced_bce_value_and_gradient(
    ...
    gradient = np.zeros(D1O_N_WEIGHTS, dtype=float)
    ...
    for operation in reversed(operations):
        ...
        if operation.kind == "ry":
            ...
            raise RuntimeError(f"Unknown D1O operation: {operation.kind}")
