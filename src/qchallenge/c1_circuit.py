...
C1_ARCHITECTURE = "C1_CAUSAL_REUPLOAD"
C1_N_QUBITS = 4
C1_N_FEATURES = 8
C1_N_WEIGHTS = 65
C1_READOUT_QUBIT = 0
...
C1_CAUSAL_FUNNEL = ((3, 2), (2, 1), (1, 0))


def c1_input_parameters() -> list[Parameter]:
    return [Parameter(f"x_{index}") for index in range(C1_N_FEATURES)]


def c1_weight_parameters() -> list[Parameter]:
    return [Parameter(f"theta_{index}") for index in range(C1_N_WEIGHTS)]


def build_c1_unitary() -> tuple[QuantumCircuit, list[Parameter], list[Parameter]]:
    """Build C1 with two complete raw-feature re-uploading rounds.
    ...
    """
    features = c1_input_parameters()
    weights = c1_weight_parameters()
    circuit = QuantumCircuit(C1_N_QUBITS, name="compliant_c1_causal_reupload")

    for block_index, block_features in enumerate(C1_REUPLOAD_BLOCKS):
    ...
    circuit.ry(weights[64], C1_READOUT_QUBIT)
    return circuit, features, weights


def build_c1_training_circuit() -> tuple[QuantumCircuit, list[Parameter], list[Parameter]]:
    circuit, features, weights = build_c1_unitary()
    circuit.measure_all()
    return circuit, features, weights


def build_c1_submission_circuit() -> tuple[QuantumCircuit, list[Parameter], list[Parameter]]:
    circuit, features, weights = build_c1_unitary()
    circuit.add_register(ClassicalRegister(1, "c"))
    circuit.measure(C1_READOUT_QUBIT, circuit.clbits[0])
    return circuit, features, weights


def c1_constraint_report(circuit: QuantumCircuit) -> dict:
...
def export_c1_submission_qasm(destination: Path) -> dict:
    circuit, _, _ = build_c1_submission_circuit()
    report = c1_constraint_report(circuit)
    if not report["passes"]:
        raise ValueError(f"C1 circuit constraint failure: {report}")
    destination.write_text(
        "// C1: raw x1..x8 enter only single-feature affine RY gates; "
