"""End-to-end training for the compliant C1R quantum circuit."""
...
from .c1r_circuit import (
    C1R_ARCHITECTURE,
    C1R_N_FEATURES,
    C1R_N_WEIGHTS,
    C1R_REUPLOAD_BLOCKS,
    build_c1r_unitary,
    export_c1r_submission_qasm,
)
from .c1r_simulator import (
    c1r_balanced_bce_value_and_gradient,
    c1r_exact_probabilities,
)
...
@dataclass(frozen=True)
class C1RCrossValidationConfig:
    train_csv: Path
    artifacts_dir: Path = Path("artifacts")
    split_seed: int = 2026
    init_seed: int = 2026
...
def c1r_random_initial_point(
    ...
    if local_scale < 0 or affine_scale_jitter < 0:
        raise ValueError("C1R initialization scales must be non-negative.")
    rng = np.random.default_rng(seed)
    weights = rng.uniform(-local_scale, local_scale, C1R_N_WEIGHTS)
    for block_index in range(len(C1R_REUPLOAD_BLOCKS)):
...
def _stratified_subsample(
    ...
    if max_rows < 20:
        raise ValueError("C1R max_rows must be at least 20.")
    rng = np.random.default_rng(seed)
...
def _c1r_bounds() -> list[tuple[float, float]]:
    bounds = [(-np.pi, np.pi) for _ in range(C1R_N_WEIGHTS)]
    for block_index in range(len(C1R_REUPLOAD_BLOCKS)):
        offset = 16 * block_index
...
def optimize_c1r_quantum_loss(
    ...
    *,
    ...
    """Optimize analytic gradients of the C1R statevector probability only."""
    initial = c1r_random_initial_point(
        seed,
    ...
    def value_and_gradient(weight_values: np.ndarray) -> tuple[float, np.ndarray]:
        ...
        value, gradient = c1r_balanced_bce_value_and_gradient(
            x, y, weight_values
        )
    ...
    initial_loss, initial_gradient = c1r_balanced_bce_value_and_gradient(
        x, y, initial
    )
    result = minimize(
        value_and_gradient,
    ...
    return selected, {
        ...
        "objective": "balanced_binary_cross_entropy_of_C1R_q0_probability",
...
def _metrics(
    ...
    *,
    ...
    exact = c1r_exact_probabilities(x, weights)
    rng = np.random.default_rng(seed)
...
def c1r_qiskit_equivalence_report(
    x: np.ndarray, weights: np.ndarray, *, max_rows: int = 16
) -> dict:
    sample = np.asarray(x[:max_rows], dtype=float)
    custom = c1r_exact_probabilities(sample, weights)
    circuit, features, parameters = build_c1r_unitary()
...
def _config_payload(config: C1RTrainConfig) -> dict:
    return {
        **asdict(config),
...
def run_c1r_screen(config: C1RTrainConfig) -> Path:
    ...
    weights, optimization, initial = optimize_c1r_quantum_loss(
        screen_x[fit],
        screen_y[fit],
        seed=config.seed,
    ...
    run_dir = config.artifacts_dir / (
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_screen_c1r_"
        f"split{split_seed}_init{config.seed}"
    )
    run_dir.mkdir(parents=True, exist_ok=False)
    payload = {
        "mode": "train_only_holdout_C1R_direct_quantum_gradient_screen",
        "architecture": C1R_ARCHITECTURE,
        ...
        "qiskit_equivalence": c1r_qiskit_equivalence_report(screen_x[valid], weights),
        "initial_feature_causal_report": c1r_feature_causal_report(screen_x, initial),
        "trained_feature_causal_report": c1r_feature_causal_report(screen_x, weights),
...
def _stratified_folds(
    ...
    if folds < 2:
        raise ValueError("C1R cross-validation requires at least two folds.")
    rng = np.random.default_rng(seed)
...
def run_c1r_cross_validation(config: C1RCrossValidationConfig) -> Path:
    """Generate genuine C1R OOF probabilities and a train-only threshold."""
    x, y, data_info = load_raw_train(config.train_csv)
    ...
    for fold_index, (fit, valid) in enumerate(splits):
        weights, optimization, _ = optimize_c1r_quantum_loss(
            x[fit],
            y[fit],
        ...
        probability = c1r_exact_probabilities(x[valid], weights)
        ...
        equivalence_reports.append(c1r_qiskit_equivalence_report(x[valid], weights))

    if not np.isfinite(oof_probability).all():
        raise RuntimeError("C1R OOF generation left one or more rows unpredicted.")
    ...
    run_dir = config.artifacts_dir / (
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_cv_c1r_"
        f"split{config.split_seed}_init{config.init_seed}"
    )
    run_dir.mkdir(parents=True, exist_ok=False)
    payload = {
        "mode": "C1R_quantum_only_out_of_fold_threshold_selection",
        "architecture": C1R_ARCHITECTURE,
        "submission_created": False,
        "config": {
            **asdict(config),
        ...
        "threshold_source": "out_of_fold_C1R_quantum_probabilities_only",
...
def run_c1r_final(config: C1RTrainConfig) -> Path:
    x, y, data_info = load_raw_train(config.train_csv)
    if config.max_rows is not None and config.max_rows < len(y):
        raise ValueError("C1R final training must use the complete public train set.")
    if not 0.0 < config.decision_threshold < 1.0:
        raise ValueError("C1R decision_threshold must be in (0, 1).")
    weights, optimization, initial = optimize_c1r_quantum_loss(
        x,
        y,
        seed=config.seed,
    ...
    run_dir = config.artifacts_dir / (
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_final_c1r_seed{config.seed}"
    )
    run_dir.mkdir(parents=True, exist_ok=False)
    constraint = export_c1r_submission_qasm(run_dir / "classifier.qasm")
    weights_payload = {
    ...
    expected_feature_uses = {
        f"x_{feature_index}": sum(
            feature_index in block for block in C1R_REUPLOAD_BLOCKS
        )
        for feature_index in range(C1R_N_FEATURES)
    }
    provenance = {
        "architecture": C1R_ARCHITECTURE,
        "weight_count": C1R_N_WEIGHTS,
        "parameter_generation": "label_independent_random_initialization_then_direct_C1R_quantum_probability_gradient_optimization",
        "initialization": {
            "seed": config.seed,
        ...
        "loss_source": "C1R q0 measurement probability versus raw public_train label",
        "loss": "balanced binary cross-entropy",
        "optimizer": "L-BFGS-B with exact adjoint quantum-circuit gradient",
        "selected_raw_features_zero_based": list(range(C1R_N_FEATURES)),
        "selected_raw_features_csv": [f"x{index + 1}" for index in range(C1R_N_FEATURES)],
    ...
    metrics = {
        "mode": "full_train_C1R_direct_quantum_gradient_optimization",
        "architecture": C1R_ARCHITECTURE,
        "config": _config_payload(config),
        "data": data_info,
        "optimization": optimization,
        ...
        "qiskit_equivalence": c1r_qiskit_equivalence_report(x, weights),
        "initial_feature_causal_report": c1r_feature_causal_report(x, initial),
        "trained_feature_causal_report": c1r_feature_causal_report(x, weights),
    }
    (run_dir / "training_metrics.json").write_text(
        json.dumps(metrics, indent=2), encoding="utf-8"
    )
    note = (
        "C1R rotated-second-upload VQC uses all raw CSV features with no preprocessing or augmentation. "
        ...
        "balanced cross-entropy of the exact C1R q0 statevector probability on public_train.csv. "
