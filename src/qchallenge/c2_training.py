"""End-to-end training for the compliant C2 quantum circuit."""
...
from .c2_circuit import (
    C2_ARCHITECTURE,
    C2_N_FEATURES,
    C2_N_WEIGHTS,
    C2_REUPLOAD_BLOCKS,
    build_c2_unitary,
    export_c2_submission_qasm,
)
from .c2_simulator import (
...
def c2_random_initial_point(
    ...
    if local_scale < 0 or affine_scale_jitter < 0:
        raise ValueError("C2 initialization scales must be non-negative.")
    rng = np.random.default_rng(seed)
    weights = rng.uniform(-local_scale, local_scale, C2_N_WEIGHTS)
    for block_index in range(len(C2_REUPLOAD_BLOCKS)):
...
def _stratified_subsample(
    ...
    if max_rows < 20:
        raise ValueError("C2 max_rows must be at least 20.")
    rng = np.random.default_rng(seed)
...
def _c2_bounds() -> list[tuple[float, float]]:
    bounds = [(-np.pi, np.pi) for _ in range(C2_N_WEIGHTS)]
    for block_index in range(len(C2_REUPLOAD_BLOCKS)):
        offset = 16 * block_index
...
def optimize_c2_quantum_loss(
    ...
    *,
    ...
    """Optimize analytic gradients of the C2 statevector probability only."""
    initial = c2_random_initial_point(
        seed,
    ...
    def value_and_gradient(weight_values: np.ndarray) -> tuple[float, np.ndarray]:
        ...
        value, gradient = c2_balanced_bce_value_and_gradient(
            x, y, weight_values
        )
    ...
    initial_loss, initial_gradient = c2_balanced_bce_value_and_gradient(
        x, y, initial
    )
    result = minimize(
        value_and_gradient,
    ...
    return selected, {
        ...
        "objective": "balanced_binary_cross_entropy_of_C2_q0_probability",
...
def _metrics(
    ...
    *,
    ...
    exact = c2_exact_probabilities(x, weights)
    rng = np.random.default_rng(seed)
...
def _config_payload(config: C2TrainConfig) -> dict:
    return {
        **asdict(config),
...
def run_c2_screen(config: C2TrainConfig) -> Path:
    x, y, data_info = load_raw_train(config.train_csv)
    selected_rows = _stratified_subsample(y, config.max_rows, config.seed)
    ...
    weights, optimization, initial = optimize_c2_quantum_loss(
        screen_x[fit],
        screen_y[fit],
        seed=config.seed,
    ...
    run_dir = config.artifacts_dir / (
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_screen_c2_seed{config.seed}"
    )
    run_dir.mkdir(parents=True, exist_ok=False)
    payload = {
        "mode": "train_only_holdout_C2_direct_quantum_gradient_screen",
        "architecture": C2_ARCHITECTURE,
        ...
        "fit_metrics": _metrics(
            screen_x[fit], screen_y[fit], weights, shots=config.shots, seed=config.seed + 1,
            threshold=config.decision_threshold,
        ),
        "validation_metrics": _metrics(
            screen_x[valid], screen_y[valid], weights, shots=config.shots, seed=config.seed + 2,
            threshold=config.decision_threshold,
        ),
        "qiskit_equivalence": c2_qiskit_equivalence_report(screen_x[valid], weights),
        "initial_feature_causal_report": c2_feature_causal_report(screen_x, initial),
        "trained_feature_causal_report": c2_feature_causal_report(screen_x, weights),
...
def _stratified_folds(
    ...
    if folds < 2:
        raise ValueError("C2 cross-validation requires at least two folds.")
    rng = np.random.default_rng(seed)
...
def run_c2_cross_validation(config: C2CrossValidationConfig) -> Path:
    """Generate genuine C2 OOF probabilities and a train-only threshold."""
    x, y, data_info = load_raw_train(config.train_csv)
    ...
    for fold_index, (fit, valid) in enumerate(splits):
        weights, optimization, _ = optimize_c2_quantum_loss(
            x[fit],
            y[fit],
        ...
        probability = c2_exact_probabilities(x[valid], weights)
        ...
        equivalence_reports.append(c2_qiskit_equivalence_report(x[valid], weights))

    if not np.isfinite(oof_probability).all():
        raise RuntimeError("C2 OOF generation left one or more rows unpredicted.")
    ...
    run_dir = config.artifacts_dir / (
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_cv_c2_"
        f"split{config.split_seed}_init{config.init_seed}"
    )
    run_dir.mkdir(parents=True, exist_ok=False)
    payload = {
        "mode": "C2_quantum_only_out_of_fold_threshold_selection",
        "architecture": C2_ARCHITECTURE,
        "submission_created": False,
        "config": {
            **asdict(config),
        ...
        "threshold_source": "out_of_fold_C2_quantum_probabilities_only",
...
def run_c2_final(config: C2TrainConfig) -> Path:
    x, y, data_info = load_raw_train(config.train_csv)
    if config.max_rows is not None and config.max_rows < len(y):
        raise ValueError("C2 final training must use the complete public train set.")
    if not 0.0 < config.decision_threshold < 1.0:
        raise ValueError("C2 decision_threshold must be in (0, 1).")
    weights, optimization, initial = optimize_c2_quantum_loss(
        x,
        y,
        seed=config.seed,
    ...
    run_dir = config.artifacts_dir / (
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_final_c2_seed{config.seed}"
    )
    run_dir.mkdir(parents=True, exist_ok=False)
    constraint = export_c2_submission_qasm(run_dir / "classifier.qasm")
    weights_payload = {
    ...
    expected_feature_uses = {
        f"x_{feature_index}": sum(
            feature_index in block for block in C2_REUPLOAD_BLOCKS
        )
        for feature_index in range(C2_N_FEATURES)
    }
    provenance = {
        "architecture": C2_ARCHITECTURE,
        "weight_count": C2_N_WEIGHTS,
        "parameter_generation": "label_independent_random_initialization_then_direct_C2_quantum_probability_gradient_optimization",
        "initialization": {
            "seed": config.seed,
        ...
        "loss_source": "C2 q0 measurement probability versus raw public_train label",
        "loss": "balanced binary cross-entropy",
        "optimizer": "L-BFGS-B with exact adjoint quantum-circuit gradient",
        "selected_raw_features_zero_based": list(range(C2_N_FEATURES)),
        "selected_raw_features_csv": [f"x{index + 1}" for index in range(C2_N_FEATURES)],
    ...
    metrics = {
        "mode": "full_train_C2_direct_quantum_gradient_optimization",
        "architecture": C2_ARCHITECTURE,
        "config": _config_payload(config),
        "data": data_info,
        "optimization": optimization,
        ...
        "qiskit_equivalence": c2_qiskit_equivalence_report(x, weights),
        "initial_feature_causal_report": c2_feature_causal_report(x, initial),
        "trained_feature_causal_report": c2_feature_causal_report(x, weights),
    }
    (run_dir / "training_metrics.json").write_text(
        json.dumps(metrics, indent=2), encoding="utf-8"
    )
    note = (
        "C2 causal data-reuploading VQC uses all raw CSV features with no preprocessing or augmentation. "
        "Blocks 1/3/5 encode x1-x4 and blocks 2/4/6 encode x5-x8 as single-feature affine "
        ...
        "balanced cross-entropy of the exact C2 q0 statevector probability on public_train.csv. "
