"""Vectorized exact statevector and adjoint gradients for the D1S circuit."""
...
_Z_READOUT_SIGN = np.where(
    ((_BASIS >> D1S_READOUT_QUBIT) & 1) == 0, 1.0, -1.0
)
...
def _validate_inputs(x: np.ndarray, weights: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    if features.ndim != 2 or features.shape[1] != D1S_N_FEATURES:
        raise ValueError(f"D1S expects shape (n, {D1S_N_FEATURES}); got {features.shape}.")
    if parameters.shape != (D1S_N_WEIGHTS,):
        raise ValueError(f"D1S expects {D1S_N_WEIGHTS} weights; got {parameters.shape}.")
    if not np.isfinite(features).all() or not np.isfinite(parameters).all():
        raise ValueError("D1S inputs and weights must be finite.")
...
def _forward_with_operations(
    ...
    for block_index, (block_features, entangler) in enumerate(
        ...
        for qubit in range(D1S_N_QUBITS):
            parameter_index = offset + 4 + qubit
        ...
        if block_index in (3, 7):
            ...
            for qubit in range(D1S_N_QUBITS):
                parameter_index = mixer_offset + 2 + qubit
                angle = float(parameters[parameter_index])
                before = state
                state = _apply_ry(state, angle, qubit)
                operations.append(
...
def d1s_exact_probabilities(x: np.ndarray, weights: np.ndarray) -> np.ndarray:
...
def _balanced_sample_weights(y: np.ndarray) -> np.ndarray:
    ...
    if not np.isin(labels, (0, 1)).all():
        raise ValueError("D1S labels must be binary 0/1 values.")
    return result


def d1s_balanced_bce_value_and_gradient(
    ...
    gradient = np.zeros(D1S_N_WEIGHTS, dtype=float)
    ...
    for operation in reversed(operations):
        ...
        if operation.kind == "ry":
            ...
            raise RuntimeError(f"Unknown D1S operation: {operation.kind}")
